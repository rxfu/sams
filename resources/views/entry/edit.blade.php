...
@section('title', __('Edit') . __('entry.module'))
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">{{ __('Edit') . __('entry.module') }}: {{ $item->getKey() }}</h3>
            </div>

		    <form role="form" id="edit-form" name="edit-form" method="post" action="{{ route('entries.update', $item) }}">
                @csrf
                @method('put')
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" placeholder="{{ __('entry.name') }}" value="{{ old('name', $item->name) }}">
                            @error('name')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description" rows="5" placeholder="{{ __('entry.description') }}">{{ old('description', $item->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </div>
